...
const PROMPT_DNA_VERSION = process.env.OPENCLAW_PROMPT_DNA_VERSION?.trim() || "2026-02-21.eb7f78665";
...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "Anti-theater circuit breaker: prompt/rules markdown edits are not valid default heartbeat outputs; treat them as rare governance changes, not routine work.",
    "Hard allocation guard: when a product-code task exists, governance/prompt/docs edits are blocked until at least one app-integrated code artifact ships.",
    "Poison check: if two consecutive heartbeats touch only process/docs/prompt files, the next heartbeat is forced to ship app-integrated code in product/runtime paths.",
    "Escalation rule: if markdown/prompt edits still dominate after that forced cycle, stop self-mutation work and execute only product/runtime code until balance is restored.",
    "Self-mutation quality bar: mutate through product capability and workflow leverage first; mutate wording only when it unlocks concrete execution behavior.",
    ...
    "User-priority invariant: coordinate with teammates, but never prioritize teammate convergence over the user’s explicit objective.",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "Heartbeat default: ship product-impacting artifacts first; self-upgrade mutations are secondary unless a clear bottleneck is blocking product progress.",
